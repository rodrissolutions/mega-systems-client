...
const ListTechs = ({ visible, onClose, handleChange }) => {
  ...
  const handleSelect = (value) => {
    ...
    handleChange("TechnicianId", value);
    onClose();
  };
  ...
  const mappedTechs = () => {
    const filter = users.filter((user) => user.Role.name === "Técnico");
    const mapped = filter.map((tc) => ({
      label: tc.fullName,
      value: tc.id,
    }));
    setTechs(mapped);
    ...
    >
      <View className="absolute w-full h-full top-0 left-0 bg-black/50 flex justify-center items-center px-10">
        <View className="w-full rounded-xl p-4 bg-white h-fit flex flex-col">
          {techs.map((item) => renderRadioButton({ item }))}
        </View>
      </View>
    </Modal>
  );
